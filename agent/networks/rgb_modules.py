...
class ResnetEncoder(nn.Module):
    ...
    def forward(self, x, cond=None, return_intermediate=False):
        # # preprocess
        ...
        h = self.block_1(h)
        if cond is not None and self.cond_fusion == "film":  # FiLM layer
            B, C, H, W = h.shape
            beta, gamma = torch.split(
                self.lang_proj1(cond).reshape(B, C * 2, 1, 1), [C, C], 1
            )
            h = (1 + gamma) * h + beta
        ...
        if cond is not None and self.cond_fusion == "film":  # FiLM layer
            B, C, H, W = h.shape
            beta, gamma = torch.split(
                self.lang_proj2(cond).reshape(B, C * 2, 1, 1), [C, C], 1
            )
            h = (1 + gamma) * h + beta
        ...
        if cond is not None and self.cond_fusion == "film":  # FiLM layer
            B, C, H, W = h.shape
            beta, gamma = torch.split(
                self.lang_proj3(cond).reshape(B, C * 2, 1, 1), [C, C], 1
            )
            h = (1 + gamma) * h + beta
        ...
        if cond is not None and self.cond_fusion == "film":  # FiLM layer
            B, C, H, W = h.shape
            beta, gamma = torch.split(
                self.lang_proj4(cond).reshape(B, C * 2, 1, 1), [C, C], 1
            )
            h = (1 + gamma) * h + beta
