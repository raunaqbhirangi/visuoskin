...
os.environ["MUJOCO_GL"] = "egl"  # "osmesa" for calvin
...
class WorkspaceIL:
    def __init__(self, cfg):
        ...
        if self.cfg.sequential_train:
            self.scene_idx = 0
            self.scene_names = self.cfg.suite.task.scenes
            self.task_names = {
                task_name: scene[task_name]
                for scene in self.cfg.suite.task.tasks
                for task_name in scene
            }
            self.envs_till_idx = len(self.task_names[self.scene_names[0]])
        ...
        # # Discretizer for BeT
        # if self.cfg.agent.policy_head in ["bet", "vqbet"]:
        #     self.agent.discretize(self.expert_replay_loader.dataset.actions)
